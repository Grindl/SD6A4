...
void main()
{
	...
	vec2 texCoordOffset = vec2((rand(vec2(texSeed.r, u_time))-.5)*.1, (rand(vec2(u_time, texSeed.b))-.5)*.1);
	...
	if(u_useNormal)
	{
		vec3 normals = texture2D(u_normalTexture, v_texCoord+texCoordOffset).rgb;
	...
	if(u_showSpecular)
	{
		...
		if(u_useSpecular)
		{
			vec4 specularData = texture2D(u_specularTexture, v_texCoord+texCoordOffset);
	...
	if(u_useEmissive)
	{
		vec4 emissiveColor = texture2D(u_emissiveTexture, v_texCoord+texCoordOffset);
